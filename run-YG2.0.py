...
class MyWindow(QtGui.QMainWindow):
    ...
    def createActions(self):
        ...
        self.manual.setShortcut("Ctrl+A")
    ...
    def dstartest(self):
        ...
        if currentest == u'左轮毂电机控制':
            ...
            result = QtGui.QMessageBox.question(self,U"提示",self.trUtf8("C板处于转发模式？"),QtGui.QMessageBox.Yes| QtGui.QMessageBox.No)
            if result == QtGui.QMessageBox.Yes:
                self.getdboardinfo()
        elif currentest == u'获取超声信息':
            self.getdboardinfo()
        else:
            pass
    ...
    def boardmess(self,mess,bid):
        if bid == '51' or bid=='52' or bid == '53' or bid=='54' or bid == '55' or bid=='56' or bid == '57' or bid=='58' or bid == '59' or bid=='5A':
            ...
            warringcode=mess[8:10]
            self.dboardChild.reginfo.append(u'warringcode:'+warringcode)
            errcode=mess[10:12]
            self.dboardChild.reginfo.append(u'warringcode:'+errcode)
            Accx=mess[12:16]
    ...
    def cutmodel(self):
        ...
        mess='59'+'EE'+'E2'+'19'+'14'+tockdata+'01000000'+'01'
        tocksend=self.messmarge(mess,1)
        self.dboardChild.testmessage.append('cmd:'+tocksend.encode('hex'))
        Ygcboardev.write(tocksend)
        self.cuter=cutmodelthread(bid,cmd)
        self.cuter.messSingnal.connect(self.dcutmess)
        self.cuter.start()
    ...
    def dotaupdate(self):
        ...
        if currentboard==u'轮毂电机':
            ...
            self.otaChild.sublist.addItem(u'直流电机驱动板一')
            self.otaChild.sublist.addItem(u'直流电机驱动板二')
            self.otaChild.sublist.addItem(u'直流电机驱动板三')
            self.otaChild.sublist.addItem(u'直流电机驱动板四')
        elif currentboard==u'左右箱体控制板':
            self.otaChild.sublist.setEnabled(True)
            self.otaChild.sublist.addItem(u'箱体广播升级')
        ...
        if currentitem==u'读取通用寄存器':
            ...
            if model:
                if currensubtitem == u'写入SN':
                    ctime=self.getcurrentime()
                    f.write(ctime+boardid+u'写入SN'+'\n')
                    cmd='F1'
                    snchar=''
    ...
    def ucboardwindow(self):
        ...
        #self.ucboardChild.testitem.itemDoubleClicked.connect(self.cstartest)
    ...
    def itemtest(self):
        ...
        f=open('log/oplog.txt','a')
        if currentsubitem == u'切换到正常':
            ctime=self.getcurrentime()
            f.write(ctime+u'切换到正常'+'\n')
            mess=str('59'+'EE'+'62'+'04'+'00'+'000000'+'01'+'90E547').decode('hex')
            Ygcboardev.write(mess)
            self.cboardChild.testmessage.append(mess.encode('hex'))
            self.cboardChild.workmodel.setText(u'正常')
        elif currentsubitem == u'切换到转发':
    ...
    def startock(self,seq):
        ...
        tockdata='000000000000000F0000000000000000'
        f=open('log/oplog.txt','a')
        if currentitem==u'读取通用寄存器信息':
            cmd='00'
            if currensubtitem == u'读取指令测试':#用于测试读指令，没有data返回值
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'读取指令测试'+'\n')
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'读取64byte'+'\n')
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'读取65byte'+'\n')
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'读取起始地址65'+'\n')
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'CRC错误帧'+'\n')
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'LEN错误帧'+'\n')
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'回复错误seq'+'\n')
                newseq='0x'+str(seq)
            ...
            if currensubtitem == u'写入硬件版本号':
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'写入硬件版本号'+'\n')
                cmd='72'
                if newhardver!='':
                    self.cboardChild.testmessage.append(u'硬件版本号长度:'+str(len(newhardver)))
                    lenhard=hex(len(newhardver)).lstrip('0x')
                    if len(lenhard)==1:
                        lenhard='0'+lenhard
                    if len(newhardver)==1:
                        newhardver='000'+newhardver
                    elif len(newhardver)==2:
                        newhardver='00'+newhardver
                    elif len(newhardver)==3:
                        newhardver='0'+newhardver
                    else:
                        pass
                    mess='59'+str(boardid)+'72'+lenhard+newhardver+seq
                    tocksend=self.messmarge(mess,1)
                    self.cboardChild.testmessage.append('tock:'+tocksend.encode('hex'))
                    Ygcboardev.write(tocksend)
                    Ygcboardev.flushInput()
                ...
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'读取硬件版本号'+'\n')
                cmd='00'
            ...
            if currensubtitem == u'读取软件版本号':
                ctime=self.getcurrentime()
                f.write(ctime+boardid+u'读取软件版本号'+'\n')
                cmd='00'
    ...
    def udptitowindow(self):
        ...
        self.udpChild.leftdoormotoropen1.clicked.connect(self.udpdoorleftopenset)
        self.udpChild.leftdoormotorclose1.clicked.connect(self.udpdoorleftcloseset)

        self.udpChild.leftdoormotoropen2.clicked.connect(self.udpdoorsecleftopenset)
        self.udpChild.leftdoormotorclose2.clicked.connect(self.udpdoorsecleftcloseset)

        self.udpChild.rightdoormotoropen1.clicked.connect(self.udpdoorightopenset)
        self.udpChild.rightdoormotorclose1.clicked.connect(self.udpdoorightcloseset)

        self.udpChild.rightdoormotoropen2.clicked.connect(self.udpdoorightsecopenset)
        self.udpChild.rightdoormotorclose2.clicked.connect(self.udpdoorightsecloseset)
    ...
    def udpdoorupopen(self):
        ...
        self.udpChild.leftdoormotoropen1.setChecked(True)
        self.udpChild.leftdoormotorclose1.setChecked(False)
        #self.titoChild.leftdoormotornon2.setChecked(False)
        self.udpChild.leftdoormotoropen2.setChecked(True)
        self.udpChild.leftdoormotorclose2.setChecked(False)
        #self.titoChild.updoornon.setChecked(False)
        self.udpChild.updoorclose.setChecked(False)

    def udpdoorupclose(self):
        #self.titoChild.leftdoormotornon1.setChecked(False)
        self.udpChild.leftdoormotoropen1.setChecked(False)
        self.udpChild.leftdoormotorclose1.setChecked(True)
        #self.titoChild.leftdoormotornon2.setChecked(False)
        self.udpChild.leftdoormotoropen2.setChecked(False)
        self.udpChild.leftdoormotorclose2.setChecked(True)
        self.udpChild.updooropen.setChecked(False)
        #self.titoChild.updoornon.setChecked(False)

    def udpdoordownopen(self):
        #self.titoChild.rightdoormotornon1.setChecked(False)
        self.udpChild.rightdoormotoropen1.setChecked(True)
        self.udpChild.rightdoormotorclose1.setChecked(False)
        #self.titoChild.rightdoormotornon2.setChecked(False)
        self.udpChild.rightdoormotoropen2.setChecked(True)
        self.udpChild.rightdoormotorclose2.setChecked(False)
        self.udpChild.downdoorclose.setChecked(False)
        #self.titoChild.downdoornon.setChecked(False)

    def udpdoordownclose(self):
        #self.titoChild.rightdoormotornon1.setChecked(False)
        self.udpChild.rightdoormotoropen1.setChecked(False)
        self.udpChild.rightdoormotorclose1.setChecked(True)
        #self.titoChild.rightdoormotornon2.setChecked(False)
        self.udpChild.rightdoormotoropen2.setChecked(False)
        self.udpChild.rightdoormotorclose2.setChecked(True)
        #self.titoChild.downdoornon.setChecked(False)
        self.udpChild.downdooropen.setChecked(False)

    def udpdoorleftopenset(self):
        #self.titoChild.leftdoormotornon1.setChecked(False)
        self.udpChild.leftdoormotorclose1.setChecked(False)

    def udpdoorleftcloseset(self):
        #self.titoChild.leftdoormotornon1.setChecked(False)
        self.udpChild.leftdoormotoropen1.setChecked(False)

    def udpdoorsecleftopenset(self):
        #self.titoChild.leftdoormotornon2.setChecked(False)
        self.udpChild.leftdoormotorclose2.setChecked(False)

    def udpdoorsecleftcloseset(self):
        self.udpChild.leftdoormotoropen2.setChecked(False)
        #self.titoChild.leftdoormotornon2.setChecked(False)

    def udpdoorightopenset(self):
        #self.titoChild.rightdoormotornon1.setChecked(False)
        self.udpChild.rightdoormotorclose1.setChecked(False)

    def udpdoorightcloseset(self):
        #self.titoChild.rightdoormotornon1.setChecked(False)
        self.udpChild.rightdoormotoropen1.setChecked(False)

    def udpdoorightsecopenset(self):
        #self.titoChild.rightdoormotornon2.setChecked(False)
        self.udpChild.rightdoormotorclose2.setChecked(False)

    def udpdoorightsecloseset(self):
        #self.titoChild.rightdoormotornon2.setChecked(False)
        self.udpChild.rightdoormotoropen2.setChecked(False)
    ...
    def udptitoany(self,messlist):
        ...
        self.udpChild.xvalue.setText(str(messlist[12]))
        self.udpChild.yvalue.setText(str(messlist[13]))
        self.udpChild.zvalue.setText(str(messlist[14]))
        ...
        if messlist[47]=='61':
            self.udpChild.leftwei1.setText(str(messlist[48]))
            self.udpChild.leftwei2.setText(str(messlist[49]))
        if messlist[47]=='62':
            self.udpChild.rightwei1.setText(str(messlist[48]))
            self.udpChild.rightwei2.setText(str(messlist[49]))
    ...
    def titowindow(self):
        ...
        self.titoChild.comnon.clicked.connect(self.noncomset)
        self.titoChild.commodel.clicked.connect(self.comset)
        ...
        self.titoChild.openrightdoorup.clicked.connect(self.doorleftopenset)
        self.titoChild.closerightdoordown.clicked.connect(self.doorleftcloseset)

        self.titoChild.openleftdoorup.clicked.connect(self.doorsecleftopenset)
        self.titoChild.closeleftdoordown.clicked.connect(self.doorsecleftcloseset)

        self.titoChild.openrightdoordown.clicked.connect(self.doorightopenset)
        self.titoChild.closerightdoordown.clicked.connect(self.doorightcloseset)

        self.titoChild.openleftdoorup.clicked.connect(self.doorightsecopenset)
        self.titoChild.closeleftdoordown.clicked.connect(self.doorightsecloseset)
    ...
    def doorleftopenset(self):
        #self.titoChild.leftdoormotornon1.setChecked(False)
        self.titoChild.closerightdoorup.setChecked(False)

    def doorleftcloseset(self):
        #self.titoChild.leftdoormotornon1.setChecked(False)
        self.titoChild.openrightdoorup.setChecked(False)
    ...
    def doorsecleftopenset(self):
        #self.titoChild.leftdoormotornon2.setChecked(False)
        self.titoChild.closeleftdoorup.setChecked(False)

    def doorsecleftcloseset(self):
        self.titoChild.openleftdoorup.setChecked(False)
        #self.titoChild.leftdoormotornon2.setChecked(False)
    ...
    def doorightopenset(self):
        #self.titoChild.rightdoormotornon1.setChecked(False)
        self.titoChild.closerightdoordown.setChecked(False)

    def doorightcloseset(self):
        #self.titoChild.rightdoormotornon1.setChecked(False)
        self.titoChild.openrightdoordown.setChecked(False)
    ...
    def doorightsecopenset(self):
        #self.titoChild.rightdoormotornon2.setChecked(False)
        self.titoChild.closeleftdoordown.setChecked(False)

    def doorightsecloseset(self):
        #self.titoChild.rightdoormotornon2.setChecked(False)
        self.titoChild.openleftdoordown.setChecked(False)
    ...
    def titoany(self,messlist):
        ...
        self.titoChild.xvalue.setText(str(messlist[12]))
        self.titoChild.yvalue.setText(str(messlist[13]))
        self.titoChild.zvalue.setText(str(messlist[14]))
        ...
        if messlist[47]=='61':
            self.titoChild.leftwei1.setText(str(messlist[48]))
            self.titoChild.leftwei2.setText(str(messlist[49]))
        if messlist[47]=='62':
            self.titoChild.rightwei1.setText(str(messlist[48]))
            self.titoChild.rightwei2.setText(str(messlist[49]))
    ...
    def tockmess(self,mess):
        ...
        if currensubtitem == u'读取指令测试':
            ...
            if mess[138:140] == '00':
                hardverinfo=str(mess[144:148])
                currenthardver=str(self.cboardChild.hardver.toPlainText())
                if hardverinfo == currenthardver:
                    self.cboardChild.testmessage.append(u'硬件版本号已读取')
                else:
                    self.cboardChild.testmessage.append(u'读取硬件版本号和输入的硬件版本号不一致')
            else:
            ...
            if mess[138:140] == '00':
                ...
                if hardverinfo == currenthardver:
                    ...
                    self.cboardChild.testmessage.append(u'读取的软件版本号和输入的软件版本号不一致')
            else:
            ...
            if mess[138:140] == '00':
                ...
                if hardverinfo == currenthardver:
                    ...
                    self.cboardChild.testmessage.append(u'读取的协议版本号和输入的协议版本号不一致')
            else:
    ...
    def otabidget(self):
        ...
        if currentstation == 'Board-C':
            ...
            bid='10'
        elif currentstation == u'开门电机':
            currentsubdevice=unicode(self.otaChild.sublist.currentItem().text())
            if currentsubdevice == u'直流电机1':
                bid='21'
            elif currentsubdevice == u'直流电机2':
                bid='22'
            elif currentsubdevice == u'直流电机3':
                bid='23'
            else:
                bid='24'
        elif currentstation == u'左箱体':
            bid='61'
        elif currentstation == u'右箱体':
            bid='62'
...
class titodebug(QtCore.QThread):
    ...
    def tocksend(self,seq,flag):
        ...
        mess='59'+bid+cmd+'11'+'10'+tockmessage+seq
    ...
    def frameMessgeanaly(self,frame):
        ...
        if bid=='61' or bid=='62':
            temp1=int((frame[144]+frame[145]+frame[142]+frame[143]),16)
            temp2=int((frame[148]+frame[149]+frame[146]+frame[147]),16)
            if temp1>32767:
                tempdata=65536-temp1
                weivalue1='-'+str(tempdata)
            else:
                weivalue1=temp1
            if temp2>32767:
                tempdata=65536-temp2
                weivalue2='-'+str(tempdata)
            else:
                weivalue2=temp2
            devicelist.append(weivalue1)
            devicelist.append(weivalue2)
        return devicelist
    ...
    def run(self):
        ...
        if result == '':
            ...
            if checkFlag ==1 :
                ...
                if result[138:140]=='00':
                    self.messSingnal.emit(u'C板已进入转发模式')
                else:
                    self.messSingnal.emit(u'C板进入转发模式失败')
...
class udpthread(QtCore.QThread):
    ...
    def tocksend(self,bidflag):
        ...
        if bidflag==0:
            ...
            bid='62'
            cmd='96'
        mess='59'+bid+cmd+'11'+'10'+tockmessage+'00'
        tocksendmess=self.messmarge(mess,1)
        return tocksendmess
    ...
    def getdoorcmd(self):
        ...
        if udpwin.groupBox_12.isEnabled():
            if udpwin.leftdoormotoropen1.isChecked():
                leftupaction=1
            else:
                leftupaction=0

            if udpwin.leftdoormotoropen2.isChecked():
                leftdownaction=4
            else:
                leftdownaction=0

            if udpwin.rightdoormotoropen1.isChecked():
                rightupaction=2
            else:
                rightupaction=0

            if udpwin.rightdoormotoropen2.isChecked():
                rightdownaction=8
            else:
                rightdownaction=0
            temp=hex(leftupaction+rightupaction+leftdownaction+rightdownaction).lstrip('0x')
...
class titoburnthread(QtCore.QThread):
    ...
    def run(self):
        for current in range(1,self.test+1):
            ...
            if burnstopexist:
                break
            titowin.leftdoormotoropen1.setChecked(True)
            titowin.leftdoormotorclose1.setChecked(False)
            titowin.leftdoormotoropen2.setChecked(True)
            titowin.leftdoormotorclose2.setChecked(False)
            ...
            titowin.rightdoormotoropen1.setChecked(True)
            titowin.rightdoormotorclose1.setChecked(False)
            titowin.rightdoormotoropen2.setChecked(True)
            titowin.rightdoormotorclose2.setChecked(False)
            ...
            titowin.leftdoormotoropen1.setChecked(False)
            titowin.leftdoormotorclose1.setChecked(True)
            titowin.leftdoormotoropen2.setChecked(False)
            titowin.leftdoormotorclose2.setChecked(True)
            ...
            titowin.rightdoormotoropen1.setChecked(False)
            titowin.rightdoormotorclose1.setChecked(True)
            titowin.rightdoormotoropen2.setChecked(False)
            titowin.rightdoormotorclose2.setChecked(True)
